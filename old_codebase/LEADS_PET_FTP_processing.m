...
for i=1:size(avftps,1)
    ...
    if size(srcftpsuvr,1)==0
        ...
        srcnu=dir(strcat(avftps{i,1},'/LDS*nu.nii'));
        nuscan=strcat(srcnu.folder,'/',srcnu.name);
        srcaparc=dir(strcat(avftps{i,1},'/LDS*raparc+aseg.nii'));
        aparcscan=strcat(srcaparc.folder,'/',srcaparc.name);
