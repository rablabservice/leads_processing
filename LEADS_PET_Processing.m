...
prosubjs=dir('LDS*'); prosubjs={prosubjs.name}'; %% list of processed subjects
...
%% Small QC module, for a given subject there is duplicate new FBB
%% This is possible if the user inadvertedly downloaded two different sequences or two repeat scans

if size(unique(listnewfbbs),1)<size(listnewfbbs,1)
    
    
   fprintf(2,'Warning! There is at least one duplicated FBB in the new batch.\nTake a look below:\n\n');
   
    [~, uniqueIdx] =unique(listnewfbbs); % Find the indices of the unique strings
    duplicates = listnewfbbs; % Copy the original into a duplicate array
    ...
    disp(duplicates);
   error('I will stop here. Clear the workspace, cleanup data in the new FBB folder and re-run.');
    
end % end if condition QC there are no duplicate FBBs
...
chkfbbs=intersect(listnewfbbs,listfbbs);
    ...
    end % end if condition some FBBs have already been processed
...
for i=1:size(newfbbs,1)
    ...
    temptps=temptps(cell2mat(temptps(:,4))==min(cell2mat(temptps(:,4))),:); % if multiple still meet criteria, pick the closest to pet
        ...
        if size(temptps,1)==1
            ...
            srcnu=dir(strcat(temptps{1,1},'/LDS*nu.nii')); nulnk=strcat(temptps{1,1},'/',srcnu.name);
            srcaparc=dir(strcat(temptps{1,1},'/LDS*raparc+aseg.nii')); aparclnk=strcat(temptps{1,1},'/',srcaparc.name);

            nucmd=strcat('cp -a',{' '},nulnk,{' '}, strcat(path_processed,temp_id,'/',temptps{1,2},'/FBB_',temp_date)); system(char(nucmd));
            aparccmd=strcat('cp -a',{' '},aparclnk,{' '},strcat(path_processed,temp_id,'/',temptps{1,2},'/FBB_',temp_date)); system(char(aparccmd));  
        ...
        end % end if condition number of timepoints meeting criteria
    ...
    end % end if condition there is at least one timepoint
    
    clear tempid temptps oldfname newfolder newfname srcnu srcaparc nulnk aparclnk nucmd aparccmd
    
end % end for loop for each of the new fbb scans
...
%% Small QC module, for a given subject there is duplicate new FTP
%% This is possible if the user inadvertedly downloaded two different sequences or two repeat scans

if size(unique(listnewftps),1)<size(listnewftps,1)
       
   fprintf(2,'Warning! There is at least one duplicated FTP in the new batch.\nTake a look below:\n\n');
   
    [~, uniqueIdx] =unique(listnewftps); % Find the indices of the unique strings
    duplicates = listnewftps; % Copy the original into a duplicate array
    duplicates(uniqueIdx) = []; % remove the unique strings, anything left is a duplicate
    duplicates = unique(duplicates); % find the unique duplicates
    disp(duplicates);
   error('I will stop here. Clear the workspace, cleanup data in the new FTP folder and re-run.'); 
  
    
end % end if condition QC there are no duplicate FTPs
...
chkftps=intersect(listnewftps,listftps);
    ...
    end % end if condition some FTPs have already been processed
...
for i=1:size(newftps,1)
    ...
    temptps=temptps(cell2mat(temptps(:,4))==min(cell2mat(temptps(:,4))),:); % if multiple still meet criteria, pick the closest to pet
        ...
        if size(temptps,1)==1
            ...
            nucmd=strcat('cp -a',{' '},nulnk,{' '}, strcat(path_processed,temp_id,'/',temptps{1,2},'/FTP_',temp_date)); system(char(nucmd));
            aparccmd=strcat('cp -a',{' '},aparclnk,{' '},strcat(path_processed,temp_id,'/',temptps{1,2},'/FTP_',temp_date)); system(char(aparccmd));  

            % Let's store the reverse normalized matrix to process the
            % SUIT template later for FTP
            
            srcrevnorm=dir(strcat(temptps{1,1},'/iy_*')); revnormlnk=strcat(temptps{1,1},'/',srcrevnorm.name);            
            revnormcmd=strcat('ln -s',{' '},revnormlnk, {' '}, strcat(path_processed,temp_id,'/',temptps{1,2},'/FTP_',temp_date,'/y_revnorm.nii')); system(char(revnormcmd));
...
end % end for loop for each of the new ftp scans
...
%% Small QC module, for a given subject there is duplicate new FDG
%% This is possible if the user inadvertedly downloaded two different sequences or two repeat scans

if size(unique(listnewfdgs),1)<size(listnewfdgs,1)

       fprintf(2,'Warning! There is at least one duplicated FDG in the new batch.\nTake a look below:\n\n');
   
    [~, uniqueIdx] =unique(listnewfdgs); % Find the indices of the unique strings
    duplicates = listnewfdgs; % Copy the original into a duplicate array
    duplicates(uniqueIdx) = []; % remove the unique strings, anything left is a duplicate
    duplicates = unique(duplicates); % find the unique duplicates
    disp(duplicates);
   error('I will stop here. Clear the workspace, cleanup data in the new FDG folder and re-run.'); 
  
    
end % end if condition QC there are no duplicate FDGs
...
chkfdgs=intersect(listnewfdgs,listfdgs);
    ...
    end % end if condition some FDGs have already been processed
...
for i=1:size(newfdgs,1)
    ...
    temptps=temptps(cell2mat(temptps(:,4))==min(cell2mat(temptps(:,4))),:); % if multiple still meet criteria, pick the closest to pet
        ...
        if size(temptps,1)==1
            ...
            srcnu=dir(strcat(temptps{1,1},'/LDS*nu.nii')); nulnk=strcat(temptps{1,1},'/',srcnu.name);
            srcaparc=dir(strcat(temptps{1,1},'/LDS*raparc+aseg.nii')); aparclnk=strcat(temptps{1,1},'/',srcaparc.name);
            srcbs=dir(strcat(temptps{1,1},'/LDS*rbrainstemSsLabels_v12_VoxSpace.nii')); bslnk=strcat(temptps{1,1},'/',srcbs.name);
            
            nucmd=strcat('cp -a',{' '},nulnk,{' '}, strcat(path_processed,temp_id,'/',temptps{1,2},'/FDG_',temp_date)); system(char(nucmd));
            aparccmd=strcat('cp -a',{' '},aparclnk,{' '},strcat(path_processed,temp_id,'/',temptps{1,2},'/FDG_',temp_date)); system(char(aparccmd));  
            bscmd=strcat('cp -a',{' '},bslnk,{' '},strcat(path_processed,temp_id,'/',temptps{1,2},'/FDG_',temp_date)); system(char(bscmd));
...
end % end for loop for each of the new fdg scans
