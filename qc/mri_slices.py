...
cmap_yellow2 = LinearSegmentedColormap.from_list(
    "custom_color",
    [(249 / 255, 195 / 255, 31 / 255), (249 / 255, 195 / 255, 31 / 255)],
    N=256,
)
...
class MRIQCplots:
    def __init__(
        ...
        if self.crop_neck and self.aparc_img is None:
            ...
            warnings.warn(
                "The aparc_img is not provided. The neck will not be cropped.",
                UserWarning,
            )
    ...
    def load_c1_nii_resliced(self, path, orientation="LAS"):
        ...
        if not os.path.exists(reslice_mask_path):
            ...
            print(
                "Resliced image already exists for ",
                id,
                ". Loading the resliced image...",
            )
    ...
    def nu_img_slices(self):
        ...
        return QCImageGenerator(
            ...
            crop_neck=self.aparc_img if self.crop_neck else None,
        ).generate_qc_images()
    ...
    def aparc_img_slices_ctx_lh(self):
        ...
        _, aparc_img_slices_ctx_lh = QCImageGenerator(
            ...
            select_sagittal_slices=self.sagittal_slices,
            select_coronal_slices=self.coronal_slices,
            mask_lower_threshold=1001,
            mask_upper_threshold=1035,
            crop_neck=self.aparc_img if self.crop_neck else None,
        ).generate_qc_images()
    ...
    def aparc_img_slices_ctx_rh(self):
        ...
        _, aparc_img_slices_ctx_rh = QCImageGenerator(
            ...
            select_sagittal_slices=self.sagittal_slices,
            select_coronal_slices=self.coronal_slices,
            mask_lower_threshold=2001,
            mask_upper_threshold=2035,
            crop_neck=self.aparc_img if self.crop_neck else None,
        ).generate_qc_images()
    ...
    def generate_subcortical_slices(self):
        ...
        return {
            key: generate_subcortical(*threshold)
            for key, threshold in threshold_ranges.items()
        }
    ...
    def affine_nu_img_slices(self):
        ...
        affine_nu_image_slices, tpm_img_slices = QCImageGenerator(
            ...
            mask_lower_threshold=0.3,
            mask_upper_threshold=1,
            width_padding=3,
        ).generate_qc_images()
    ...
    def plot_mri_slices(self, axes):
        ...
        sns.heatmap(
            self.nu_img_slices(), cmap="gray", vmax=mri_vmax, cbar=False, ax=axes
        )
        # Plotting the lines representing the slices
        sns.heatmap(
            self.nu_img_lines(), cmap=cmap_yellow2, vmin=0.5, cbar=False, ax=axes
        )
        axes.text(10, 30, "L", fontsize=10, color="white")
        axes.text(150, 30, "R", fontsize=10, color="white")
        axes.set_title(
            f"{self.nu_img_filename}", fontsize=10, color="white", loc="left"
        )
        axes.axis("off")
    ...
    def plot_nu_img_aparc_slices(self, axes):
        ...
        sns.heatmap(
            self.nu_img_lines(), cmap=cmap_yellow2, vmin=0.5, cbar=False, ax=axes[0]
        )
        ...
        sns.heatmap(
            self.aparc_img_slices_ctx_lh(),
            cmap=cmap_red,
            vmax=1,
            mask=self.aparc_img_slices_ctx_lh() == 0,
            cbar=False,
            ax=axes[1],
        )
        # Adding the aparc+aseg slices for right hemisphere
        sns.heatmap(
            self.aparc_img_slices_ctx_rh(),
            cmap=cmap_red,
            vmax=1,
            mask=self.aparc_img_slices_ctx_rh() == 0,
            cbar=False,
            ax=axes[1],
        )
        ...
        axes[1].set_title(
            f"Overlay: {self.aparc_img_filename}",
            fontsize=10,
            color="white",
            loc="left",
        )
    ...
    def plot_c1_img_slices(self, axes):
        ...
        sns.heatmap(
            self.c1_image_slices(),
            cmap=cmap_red,
            vmax=1,
            mask=self.c1_image_slices() == 0,
            cbar=False,
            ax=axes,
        )
        axes.set_title(
            f"Overlay: {self.c1_img_filename} (voxels > 0.3)",
            fontsize=10,
            color="white",
            loc="left",
        )
    ...
    def plot_affine_nu_img_slices(self, axes):
        ...
        axes.set_title(
            f"Underlay: {self.affine_nu_img_filename}\nOverlay: TPM.nii (c1, voxels > 0.3)",
            fontsize=10,
            color="white",
            loc="left",
        )
    ...
    def plot_warped_nu_img_slices(self, axes):
        ...
        axes.set_title(
            f"Underlay: {self.warped_nu_img_filename}\nOverlay: TPM.nii (c1, voxels > 0.3)",
            fontsize=10,
            color="white",
            loc="left",
        )
    ...
    def plot_slices(self, output_path):
        ...
        if (
            self.nu_img is not None
            and self.aparc_img is None
            and self.c1_img is None
            and self.affine_nu_img is None
            and self.warped_nu_img is None
        ):
        ...
        # If nu_img, aparc_img, c1_img, and affine_nu_img are provided
            ...
            self.aparc_img is not None
            and self.c1_img is not None
            and self.affine_nu_img is not None
            and self.warped_nu_img is None
        ):
        ...
        # If nu_img, aparc_img, c1_img, affine_nu_img, and warped_nu_img are provided
            ...
            and self.c1_img is not None
            and self.affine_nu_img is not None
            and self.warped_nu_img is not None
        ):
