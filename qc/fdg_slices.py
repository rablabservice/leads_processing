...
class FDGQCplots:
    ...
    def plot_reference_region_slices(self, axes):
        pons_ref_slices = self.reference_region_slices(self.reference_region_1)

        sns.heatmap(self.nu_img_slices(), cmap='gray', vmax=mri_vmax, cbar=False, ax=axes)

        sns.heatmap(pons_ref_slices, cmap=cmap_orange, vmin=0.1, alpha=0.3, cbar=False, mask=pons_ref_slices==0, ax=axes)
        sns.heatmap(ImageProcessor.contour_image(ImageProcessor.mask_image(pons_ref_slices, lower_threshold = 0.1, upper_threshold=pons_ref_slices.max())), cbar = False, cmap = cmap_orange, vmin = 0.1, ax = axes)
    ...
    def plot_slices(self,output_path):
        ...
        if self.suvr_img is not None and self.nu_img is None and self.c1_img is None and self.reference_region_1 is None and self.affine_suvr_img is None and self.warped_suvr_img is None:
            fig, axes = plt.subplots(1, 1, figsize=(12, 2.5))
            self.plot_suvr_slices(axes)
            add_colorbar(fig, axes[2], cmap='turbo', vmin=0, vmax=2.2, ticks=[0, 2.2], orientation='horizontal', cbar_width=0.13, cbar_height=0.01, cbar_x=0.76)
        ...
        elif self.suvr_img is not None and self.nu_img is not None and self.c1_img is None and self.reference_region_1 is None and self.affine_suvr_img is None and self.warped_suvr_img is None:
        ...
        # If the suvr_img, nu_img, and c1_img are provided
        elif self.suvr_img is not None and self.nu_img is not None and self.c1_img is not None and self.reference_region_1 is None and self.affine_suvr_img is None and self.warped_suvr_img is None:
            ...
            add_colorbar(fig, axes[3], cmap='gray', vmin=0, vmax=2.2, ticks=[0, 2.2], orientation='horizontal', cbar_width=0.13, cbar_height=0.01, cbar_x=0.76)
        ...
        elif self.suvr_img is not None and self.nu_img is not None and self.c1_img is not None and self.reference_region_1 is not None and self.affine_suvr_img is not None and self.warped_suvr_img is not None:
        ...
        # Adding '_qc' to the file name at the end
