...
class FTPQCplots:
    ...
    # _____________________________________________________________ IMAGE PROCESSING _____________________________________________________________ #
    def petvmax(self):
        """
        Determines the maximum value for the PET image based on the 99.5th percentile of the whole suvr pet image

        Returns
        -------
        float
            The maximum value for the PET image.
        
        """
        
        import numpy as np
        suvr_img = self.load_nii(self.suvr_img)
        # Removing the nan values from the image
        suvr_img = suvr_img[~np.isnan(suvr_img)]
        vmax = np.percentile(suvr_img, 99.5)
    ...
    def plot_reference_region_slices(self, axes):
        ...
        erodedwm_ref_slices = self.reference_region_slices(self.reference_region_2)
